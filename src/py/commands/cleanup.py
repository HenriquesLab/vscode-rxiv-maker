...
class CleanupManager:
    ...
    def __init__(self, manuscript_path: str = None, output_dir: str = "output", 
                 verbose: bool = False):
        ...
        """
        self.manuscript_path = manuscript_path or os.getenv("MANUSCRIPT_PATH", "MANUSCRIPT")
            ...
            for item in self.figures_dir.iterdir():
                if item.is_dir():
                    for pattern in figure_patterns:
                        for file in item.glob(pattern):
                            try:
                                ...
                                self.log(f"Failed to remove {item.name}/{file.name}: {e}", "WARNING")
                    ...
                    try:
                        ...
                        if self.verbose:
                            self.log(f"Could not remove directory {item.name}: {e}", "WARNING")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Cleanup manager for Rxiv-Maker"
    )
    parser.add_argument(
        "--manuscript-path",
        default=None,
    ...
    parser.add_argument(
        "--figures-only",
        action="store_true",
        help="Only clean generated figures"
    )
    parser.add_argument(
        "--output-only",
        action="store_true",
