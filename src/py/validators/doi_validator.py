...
class DOIValidator(BaseValidator):
    ...
    def __init__(self, manuscript_path: str, enable_online_validation: bool = True, cache_dir: Optional[str] = None):
    ...
    def _fetch_metadata_parallel(self, doi: str) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
    ...
    def _fetch_joss_metadata(self, doi: str) -> Optional[Dict[str, Any]]:
        ...
        if 'year' in bib_entry and 'year' in joss_metadata:
            ...
            if bib_year_str != joss_year:
                errors.append(
                    self._create_error(
                        ValidationLevel.WARNING,
                        f"Publication year mismatch (JOSS)",
                        file_path=bib_file,
                        line_number=line_number,
        ...
        if 'year' in bib_entry and 'publicationYear' in datacite_metadata:
            ...
            if bib_year_str != datacite_year:
                errors.append(
                    self._create_error(
                        ValidationLevel.WARNING,
                        f"Publication year mismatch (DataCite)",
                        file_path=bib_file,
                        line_number=line_number,
        ...
        if 'author' in bib_entry and 'creators' in datacite_metadata:
            ...
            if datacite_creators:
                ...
                if isinstance(first_creator, dict):
                    ...
                    first_author_family = (
                        first_creator.get('familyName') or
                        first_creator.get('family') or
                        first_creator.get('name')
                    )
                else:
                ...
                if first_author_family and first_author_family.lower() not in bib_entry['author'].lower():
                    errors.append(
                        self._create_error(
                            ValidationLevel.INFO,
                            f"First author mismatch (DataCite)",
                            file_path=bib_file,
                            line_number=line_number,
    ...
    def _identify_doi_registrar(self, doi: str) -> str:
        ...
        if doi.startswith('10.21105/joss.'):
            return ("This is a JOSS (Journal of Open Source Software) DOI. JOSS DOIs are indexed in CrossRef. "
                   "Check internet connection or verify the DOI is correct.")
        elif doi.startswith('10.5281/zenodo'):
            return ("This is a Zenodo DOI (DataCite registrar). "
                   "Verify the DOI resolves at https://doi.org/" + doi)
        elif doi.startswith('10.1101/'):
            return ("This is a bioRxiv/medRxiv preprint DOI. These may not be indexed in CrossRef depending on publication status. "
                   "Check if the preprint exists at the respective platform.")
        elif doi.startswith('10.48550/arXiv') or 'arxiv' in doi.lower():
            return ("This is an arXiv DOI. arXiv papers may not be indexed in CrossRef. "
                   "Verify the paper exists on arXiv.org.")
        elif doi.startswith('10.6084/m9.figshare'):
            return ("This is a Figshare DOI (DataCite registrar). "
                   "Verify the DOI resolves at https://doi.org/" + doi)
        elif doi.startswith('10.5194/'):
            return ("This is a Copernicus Publications DOI. Some may not be in CrossRef depending on the journal. "
                   "Check if the publication exists on the publisher's website.")
        elif doi.startswith('10.1038/'):
            return ("This is a Nature Publishing Group DOI. It should be in CrossRef if published. "
                   "Check internet connection or verify the DOI is correct.")
        elif doi.startswith('10.31219/osf.io'):
            return ("This is an OSF (Open Science Framework) preprint DOI. These are indexed by DataCite. "
                   "Check if the preprint exists at https://osf.io/preprints/")
        elif doi.startswith('10.12688/f1000research'):
            return ("This is an F1000Research DOI. These should be indexed by CrossRef. "
                   "Check internet connection or verify the DOI is correct.")
        elif any(prefix in doi for prefix in ['10.1016/', '10.1371/', '10.1126/', '10.1073/', '10.1109/']):
            return ("This is from a major publisher and should be in CrossRef if published. "
                   "Check internet connection, verify the DOI is correct, or try again later.")
        else:
            return ("This DOI may be from a specialized registrar. We checked CrossRef, DataCite, and JOSS. "
                   "Verify the DOI resolves at https://doi.org/" + doi)
    ...
    def _get_api_error_suggestion(self, error_message: str, registrar_info: str) -> str:
        ...
        if 'does not exist' in error_lower or 'not found' in error_lower:
            ...
            return ("Network connectivity issue. Check internet connection and try again. " + 
                   "If the problem persists: " + registrar_info)
        elif 'rate limit' in error_lower or 'too many requests' in error_lower:
            return ("CrossRef API rate limit exceeded. Wait a moment and try again. " +
                   "If this persists: " + registrar_info)
        else:
            return ("CrossRef API error. " + registrar_info + " " +
